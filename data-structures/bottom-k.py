...
class User:
    ...
    def __lt__(self, elem):
        if self.rating != elem.rating:
            return not(self.rating < elem.rating)
        elif self.last_active != elem.last_active:
            return not(self.last_active < elem.last_active)
        else:
            return not(self.user_id > elem.user_id)
...
"""
Takeaways
 - Python only requires you to implement __lt__
 - Heaps, sorted, bisect, all use __lt__
"""